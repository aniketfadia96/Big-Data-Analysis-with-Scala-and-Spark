...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =  {
    rdd.map(article => (article, if(article.mentionsLanguage(lang)) 1 else 0))
      .aggregate(0)((acc,v) => acc+v._2, _ + _)
  }
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    langs.map(str => (str, occurrencesOfLang(str, rdd)))
      .sortWith(_._2 > _._2)
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    rdd.flatMap(article => langs.filter(l => article.mentionsLanguage(l)).map(item => (item, 1)))
      .reduceByKey(_+_)
      .sortBy(-_._2)
      .collect
      .toList
